...
def conditions(le, key="needs"):
    return [c.strip() for need in le.get(key, []) for c in need.split("|")]
...
class FilesTest(unittest.TestCase):
    ...
    def test_conditions_name_real_things(self):
        ...
        for le in LESSONS:
            self.assertTrue(le.get("masters"), f"{le['id']}: what shows it's mastered")
            for cond in conditions(le) + conditions(le, "masters"):
                kind, *args = cond.split()
                with self.subTest(lesson=le["id"], cond=cond):
                    self.assertNotEqual(kind, "lesson", "owner: lessons open with achievements, not other lessons")
                    if kind == "won":
...
class ReaderTest(unittest.TestCase):
    ...
    def test_read_a_lesson_to_the_end(self):
        lib = reader.Library(LESSONS)
        self.assertEqual(lib.lessons[lib.pos]["id"], "command_line")      # the next step is selected
        lib.key("enter")
        self.assertEqual(lib.lesson["id"], "command_line")
        for _ in BY_ID["command_line"]["pages"]:
            lib.key("enter")
        self.assertIsNone(lib.lesson)
        self.assertEqual(state.load()["lessons"]["read"], ["command_line"])
        self.assertIn("Now practice it", lib.message)
        self.assertFalse(lesson.unlocked(state.load(), BY_ID["paths"]))       # reading opens nothing

    def test_the_cursor_starts_on_the_next_step(self):
