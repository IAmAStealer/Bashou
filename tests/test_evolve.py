...
def evolving_kitten():
    s = state.default()
    s["starter"], s["achievements"] = "cat", [f"a{i}" for i in range(14)]
...
class AnimationTest(unittest.TestCase):
    def test_old_and_new_shapes_then_the_new_pet(self):
        ...
        self.assertIn("Kitten is evolving", shown[0][0])
        ...
        self.assertIn("Kitten evolved into", shown[-1][2])
        self.assertIn("Cat", shown[-1][2])
...
class FormSwitchTest(TempState):
    def test_f_cycles_the_forms_reached(self):
        ...
        self.assertEqual(names, ["Kitten", "Cat", "Lion"])
